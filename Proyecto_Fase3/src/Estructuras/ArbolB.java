...
public class ArbolB {
    public class Nodo{
        ...
        public String name, password, username, email, telefono, direccion;
        public ArbolABB arbol_binario;
        public ArbolAVL arbol_avl;
        public DoubleListAlbumes albumes;
        Nodo siguiente, anterior;
        B_Branch derecha, izquierda;
        public Nodo(long dpi, String name, String username, String email, String password, String telefono, String direccion, int id_municipio){
            this.dpi = dpi;
            this.name = name;
    ...
    public boolean insertar(long dpi, String name, String username, String email, String password, String telefono, String direccion, int id_municipio) {
        boolean existe = false;
        password = BCrypt.hashpw(password, BCrypt.gensalt());
        Nodo nodo = new Nodo(dpi, name, username, email, password, telefono, direccion, id_municipio);
        if (raiz == null) {
            raiz = new B_Branch();
            raiz.insertar(nodo);
        } else {
    ...
    private Nodo dividir(B_Branch rama) {
        long val = -999;
        String name = " ", username = "", email = "", password = "", telefono = "", direccion = "";
        int id_municipio = 0;
        Nodo temp, Nuevito;
        ...
        while (aux != null) {
            ...
            if (cont < 3) {
                temp = new Nodo(aux.dpi, aux.name, aux.username, aux.email, aux.password, aux.telefono, aux.direccion, aux.id_municipio);
        ...
        Nuevito = new Nodo(val, name, username, email, password, telefono, direccion, id_municipio);
    ...
    private boolean busqueda_para_login(String username, String password, B_Branch root) {
        if (root != null) {
            if (!raiz.leaf) {
                busqueda_para_login(username, password,root.primero.izquierda);
                if (root.primero.siguiente != null) {
                    busqueda_para_login(username, password,root.primero.siguiente.izquierda);
                    busqueda_para_login(username, password,root.primero.siguiente.derecha);                        
                }else{
                    busqueda_para_login(username, password,root.primero.derecha);
                }
                Nodo aux = root.primero;
                while (aux != null) {
                    if (username.equals(aux.username) && BCrypt.checkpw(password, aux.password)) {
                        encontrado = true;
                    }                    
                    aux = aux.siguiente;
                }
    ...
    public boolean realizar_login(String username, String password) {
        return busqueda_para_login(username, password, raiz);
    }
    ...
    private void busqueda_para_datos(String username,B_Branch root) {
        if (root != null) {
            if (!raiz.leaf) {
                busqueda_para_datos(username,root.primero.izquierda);
                if (root.primero.siguiente != null) {
                    busqueda_para_datos(username,root.primero.siguiente.izquierda);
                    busqueda_para_datos(username,root.primero.siguiente.derecha);                        
                }else{
                    busqueda_para_datos(username,root.primero.derecha);
                }
                Nodo aux = root.primero;
                while (aux != null) {
    ...
    public Nodo realizar_visualizacion(String username) {
        temp = null;
        busqueda_para_datos(username, raiz);
        return temp;
    }
    ...
    private void generar_graphviz() {
        String ubicacion_dot = "C:\\Users\\denni\\Documents\\Varios_Progra\\EDD_PROYECTOS\\FASE_3\\Graphviz\\Archivos_TXT\\ArbolB.txt";
        String ubicacion_img = "C:\\Users\\denni\\Documents\\Varios_Progra\\EDD_PROYECTOS\\FASE_3\\Graphviz\\Imagenes\\Arboles\\B\\ArbolB.png";
